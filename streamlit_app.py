...
import json
import os
from app import load_hospital_state
...
# =========================
st.header("🏥 Hospital Capacity & Risk Dashboard")
...
with b1:
    ...
    st.metric(
        "ICU Beds",
        f"{state['beds']['icu_occupied']} / {state['beds']['icu_total']}"
    )
    st.metric(
        "General Beds",
        f"{state['beds']['general_occupied']} / {state['beds']['general_total']}"
    )

with b2:
    st.subheader("🫀 Condition Distribution")
    st.json(state["condition_counts"])

st.subheader("👥 Age Group Distribution")
st.json(state["age_groups"])
...
# 🔑 attach condition so decision_agent can enforce hard overrides
    ...
    bed_type = "ICU" if decision == "Emergency" else "General"
...
# 🔑 THIS IS WHAT YOU WERE MISSING
    update_hospital_state(patient, decision)
