...
# =========================
load_dotenv()
api_key = "(add your key here and proceed)"
if not api_key:
...
HOSPITAL_STATE_FILE = "hospital_state.json"
...
# =========================
# HOSPITAL STATE (BEDS / STATS)
# =========================
DEFAULT_HOSPITAL_STATE = {
    "beds": {
        "icu_total": 40,
        "icu_occupied": 0,
        "general_total": 60,
...
def load_hospital_state():
    if not os.path.exists(HOSPITAL_STATE_FILE):
        with open(HOSPITAL_STATE_FILE, "w") as f:
            json.dump(DEFAULT_HOSPITAL_STATE, f, indent=2)
        return DEFAULT_HOSPITAL_STATE.copy()
    with open(HOSPITAL_STATE_FILE, "r") as f:
        return json.load(f)

def save_hospital_state(state):
    with open(HOSPITAL_STATE_FILE, "w") as f:
        json.dump(state, f, indent=2)

def update_hospital_state(patient, decision):
    state = load_hospital_state()
    condition = patient["Condition"].lower()
    age = patient["Age"]

    # 🛏️ Bed allocation
    if decision == "Emergency":
        state["beds"]["icu_occupied"] += 1
    else:
        state["beds"]["general_occupied"] += 1

    # 🫀 Condition tracking
    if "heart" in condition:
        state["condition_counts"]["heart_attack"] += 1
    elif "cancer" in condition or "tumor" in condition:
        state["condition_counts"]["cancer"] += 1
    elif "infection" in condition or "fever" in condition:
        state["condition_counts"]["infection"] += 1
    else:
        state["condition_counts"]["other"] += 1

    # 👥 Age groups
    if age < 18:
        state["age_groups"]["child"] += 1
    elif age >= 40:
        state["age_groups"]["elderly"] += 1
    else:
        state["age_groups"]["adult"] += 1

    save_hospital_state(state)
...
def risk_agent(age, condition, vitals=None):
    ...
    return {"risk": risk, "score": round(score, 2), "condition": c}
def ensure_history(patient):
    """
    Guarantees patient always has a History field.
    This prevents Streamlit reruns from resurrecting history.
    """
    if "History" not in patient or patient["History"] is None:
        patient["History"] = []
    return patient
